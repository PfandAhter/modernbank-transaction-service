...
public interface TransactionRepository extends JpaRepository<Transaction, String> {
        ...
        Page<Transaction> findAllByAccountIdAndTypeAndDateBetween(@Param("accountId") String accountId,
        ...
        Page<Transaction> findAllByAccountIdInAndTypeAndDateBetween(
                        List<String> accountIds, TransactionType type, LocalDateTime start, LocalDateTime end,
                        Pageable pageable);
        ...
        Double sumWithdrawalsLast24Hours(@Param("accountId") String accountId,
                        @Param("fromTime") LocalDateTime fromTime);

        @Query("SELECT COALESCE(AVG(t.amount), 0) FROM Transaction t WHERE t.accountId = :senderAccountId AND t.date >= :since")
        Double findAvgAmountSinceBySender(@Param("senderAccountId") String senderAccountId,
        ...
        boolean existsByAccountIdAndReceiverIbanAndDateBefore(String senderAccountId, String receiverIban,
                        LocalDateTime before);

        @Query("SELECT COUNT(t) FROM Transaction t WHERE t.accountId IN :accountIds")
        Integer countAllByAccountIdIn(@Param("accountIds") List<String> accountIds);

        @Query("SELECT COALESCE(AVG(t.amount), 0) FROM Transaction t WHERE t.accountId IN :accountIds")
        Double getAverageAmountByAccountIdIn(@Param("accountIds") List<String> accountIds);
}